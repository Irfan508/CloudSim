...
public class KhalidEx {
	...
	private static List<Vm> createVM(int userId, int vms, int idShift) {
		...
		//1GHZ,500mHZ,2GHZ,1GHZ,700mhz
		int new_arr[]= {10,5,20,11,7,12,15,22,4,6};		//for all my experiments
		for(int i=0;i<vms;i++){
//			vm[i] = new Vm(idShift + i, userId, mips+(new_arr[i%10]*5), pesNumber, ram, bw, size, vmm, new CloudletSchedulerSpaceShared());
			vm[i] = new Vm(idShift + i, userId, mips*(new_arr[i]), pesNumber, ram, bw, size, vmm, new CloudletSchedulerSpaceShared());
			list.add(vm[i]);
	...
	private static List<Cloudlet> createCloudlet(int userId, int cloudlets, int idShift){
		...
		int arr[]= {1,2,3,4,5};		//for all my experiments
...
//			cloudlet[i] = new Cloudlet(idShift + i, rand.nextInt(9000)+1000, pesNumber, fileSize+(arr[(i%5)]*20), outputSize+(arr[(i%5)]*20), utilizationModel, utilizationModel, utilizationModel);
			cloudlet[i] = new Cloudlet(idShift + i, length, pesNumber, fileSize+(arr[(i%5)]*20), outputSize+(arr[(i%5)]*20), utilizationModel, utilizationModel, utilizationModel);
